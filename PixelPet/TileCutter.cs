...
namespace PixelPet {
	...
	public class TileCutter {
		...
		public TileCutter(Color emptyColor)
			: this(8, 8, emptyColor) { }
		/// <summary>
		...
		/// <param name="tileHeight">The height of the produced tiles in pixels.</param>
		/// <param name="emptyColor">The empty color to use for out-of-bounds pixels.</param>
		public TileCutter(int tileWidth, int tileHeight, Color emptyColor) {
			if (tileWidth < 1)
		...
		public Tile CutTile(Bitmap bmp, int tx, int ty) {
			return this.CutTiles(bmp, tx, ty, 1, 1).First();
		}

		private Tile CutTile(Bitmap bmp, BitmapData bmpData, int[] bmpBuffer, int ti, int tj) {
			Tile tile = new Tile(this.TileWidth, this.TileHeight);
			int[] pixels = new int[tile.PixelCount];
